...
return function(context, players, currency, amount)
	for _, player in pairs(players) do
		if CurrencyService:DoesCurrencyExist(player, currency) then
			local success = CurrencyService:Subtract(player, currency, amount)

			if success then
				context:Reply("Successfully removed " .. Utils.formatNumber(amount, 1000) .. " from " .. player.Name .. "'s " .. currency .. "!")
			else
